...
# Note: we enable `pfifo` queue discipline (qdisc) on the link from `r1` to
# `r2`. Default configuration of `pfifo` in Linux is used. For more details
# about `pfifo` qdisc in Linux, use this command on CLI: `man tc-pfifo`.
eth1.set_attributes("1000mbit", "1ms")  # from `h1` to `r1`
eth2.set_attributes("1000mbit", "1ms")  # from `h2` to `r1`
etr1c.set_attributes("10mbit", "10ms", "pfifo")  # from `r1` to `r2`
...
exp = Experiment("tcp-udp-point-to-point")
