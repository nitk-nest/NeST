...
exp = Experiment("tcp-cubic-tfo-point-to-point-3")
...
# Configure the TCP parameter to disable TFO by setting the parameter value to 0
# Note: TFO is initiated by sender during the packet transfer. Hence, we disable
# TFO only for h1.
# TFO can take values 0, 1, 2, 3 in Linux kernel. To disable TFO, set the value
# to 1. To enable TFO - set it to 1, if your machine is a client; set it to 2,
# if your machine is a server; set it to 3, if your machine is a client as well
# as a server.
h1.configure_tcp_param("fastopen", "0")
...
# Configure the parameter back to its default value
h1.configure_tcp_param("fastopen", "1")
