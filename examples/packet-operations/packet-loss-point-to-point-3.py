...
# Set the random packet loss rate to 10% on the link from `r1` to `r2`.
etr1b.set_packet_loss("10%")
...
# `Ping` from `h1` to `h2`.
# Note: You may not see a packet loss rate of 10% because we transmit only 20
# ping packets in this example. If we transmit large number of ping packets,
# the average packet loss rate observed would be 10%.
