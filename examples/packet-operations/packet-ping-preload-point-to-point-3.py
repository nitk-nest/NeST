...
# `Ping` from `h1` to `h2`.
# Send 20 ping packets from h1 to h2, but send 10 of them with the preload
# option set so that they are sent out as fast as possible, without waiting
# for a ping response.
h1.ping(eth2.address, preload=10, packets=20)
