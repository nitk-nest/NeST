...
# `two-lans-connected-via-router.py` available in `examples/basic-examples`, the
# only difference is that we use an address helper in this program to assign
# IPv4 addresses to interfaces instead of manually assigning them. Note that two
# packages: `Network` and `AddressHelper` are imported in this program (Lines 8-9 above).
...
(ets1d, etr1) = connect(s1, r1, network=n1)
(ets2d, etr2) = connect(s2, r1, network=n2)
