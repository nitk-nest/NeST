...
# Configure the parameters of `fq_pie` qdisc.  For more details about `fq_pie`
# in Linux, use this command on CLI: `man tc-fq_pie`.
qdisc = "fq_pie"
fq_pie_parameters = {
...
# Note: we enable `fq_pie` queue discipline on the link from `r1` to `r2`.
eth1.set_attributes("1000mbit", "1ms")  # from `h1` to `r1`
eth2.set_attributes("1000mbit", "1ms")  # from `h2` to `r1`
etr1c.set_attributes("10mbit", "10ms", qdisc, **fq_pie_parameters)  # `r1` to `r2`
...
exp = Experiment("fq-pie-point-to-point")
...
# Enable collection of stats for `fq_pie` qdisc installed on `etr1c` interface,
# connecting `r1` to `r2`.
