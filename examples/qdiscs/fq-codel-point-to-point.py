...
# Configure the parameters of `fq_codel` qdisc.  For more details about
# `fq_codel` in Linux, use this command on CLI: `man tc-fq_codel`.
qdisc = "fq_codel"
fq_codel_parameters = {
    "limit": "100",  # set the queue capacity to 100 packets
    "flows": "2",  # set the number of flows to 2
...
# Note: we enable `fq_codel` queue discipline on the link from `r1` to `r2`.
eth1.set_attributes("1000mbit", "1ms")  # from `h1` to `r1`
eth2.set_attributes("1000mbit", "1ms")  # from `h2` to `r1`
etr1c.set_attributes("10mbit", "10ms", qdisc, **fq_codel_parameters)  # `r1` to `r2`
...
exp = Experiment("fq-codel-point-to-point")
...
# Enable collection of stats for `fq_codel` qdisc installed on `etr1c`
# interface, connecting `r1` to `r2`.
