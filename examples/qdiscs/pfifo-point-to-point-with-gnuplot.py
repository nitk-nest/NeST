...
qdisc = "pfifo"
pfifo_parameters = {"limit": "100"}  # set the queue capacity to 100 packets
...
etr1c.set_attributes("10mbit", "10ms", qdisc, **pfifo_parameters)  # from `r1` to `r2`
...
exp = Experiment("pfifo-point-to-point")

# Configure one flow from `h1` to `h3` and another from `h2` to `h4`.
flow1 = Flow(h1, h3, eth3.get_address(), 0, 10, 1)
flow2 = Flow(h2, h4, eth4.get_address(), 0, 10, 1)
...
# Enable collection of stats for `pfifo` qdisc installed on `etr1c` interface,
# connecting `r1` to `r2`.
