...
# Note: we enable `pie` queue discipline on the link from `r1` to `r2`.
eth1.set_attributes("1000mbit", "1ms")  # from `h1` to `r1`
eth2.set_attributes("1000mbit", "1ms")  # from `h2` to `r1`
etr1c.set_attributes("10mbit", "10ms", qdisc, **pie_parameters)  # from `r1` to `r2`
...
exp = Experiment("pie-point-to-point")
...
# Enable collection of stats for `pie` qdisc installed on `etr1c` interface,
# connecting `r1` to `r2`.
