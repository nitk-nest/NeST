...
An interface can be assigned network addresses (i.e., IPv4 or IPv6 addresses).
...
is run on topologies built using topology module, in the aim of understanding
...
length, etc). This data is obtained by parsing the output of ss and tc.
...
networking tools. To keep track of all these dependencies, and to make it
...
ensures is that no two networking entities (eg. network namespaces, network
