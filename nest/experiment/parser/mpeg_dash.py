...
class MpegDashRunner(Runner):
    ...
    def parse(self):
        ...
        # The dump file name is of the form: 'mpeg_dash_gpac_{server_ns}_to_{client_ns}.log'
        # since the server sends the video to the client. The raw GPAC logs are dumped to this
        # file incase the user wants to parse the logs manually.
        dump_filename = "mpeg_dash_gpac_"
        dump_filename += f"{TopologyMap.get_node(self.dst_ns).name}_to_"
        dump_filename += f"{TopologyMap.get_node(self.ns_id).name}.log"
        ...
        chunk_number = 0
        audio_stats_list = [{"server_node": TopologyMap.get_node(self.dst_ns).name}]
        for log in self.audio_logs:
            if not log.bitrate_switch:
        ...
        server_ip = self.destination_address.get_addr(with_subnet=False)

        MpegDashResults.add_result(
            self.ns_id,
            {
                server_ip: {
                    "video": video_stats_list,
                    "audio": audio_stats_list,
                    "video_summary": video_stats_summary,
                    "audio_summary": audio_stats_summary,
                }
            },
        )
